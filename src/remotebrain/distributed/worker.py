...
def prep_model(
    ckpt_path: str, device: torch.device, target_shape: Tuple[int, int, int]
) -> PreprocessedSemanticSegmentationUnet:
...
class DistributedWorker:
    ...
    def __init__(
        self, gpu_id: int, model_ckpt_path: str, target_shape: Tuple[int, int, int]
    ):
        self.gpu_id = gpu_id
        self.device = torch.device(f"cuda:{gpu_id}")
        self.model = prep_model(model_ckpt_path, self.device, target_shape)
